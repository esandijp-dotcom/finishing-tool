...
# Every render preset a stock DaVinci Resolve install ships with —
# GetRenderPresetList() returns these mixed in with a project's own
# custom presets, with no API-level flag to tell them apart. Hardcoded
# from a live install rather than derived, since there's no other way to
# get this list; if Blackmagic adds/renames stock presets in a future
# Resolve version, a new one could slip through unfiltered (harmless —
# it just shows up as an extra dropdown option) or a renamed one could
# briefly disappear from this list (equally harmless, since filtering
# only affects what's DISPLAYED, not what LoadRenderPreset can load).
DAVINCI_BUILTIN_RENDER_PRESETS = frozenset([
    "H.264 Master", "HyperDeck", "H.265 Master", "ProRes 422 HQ",
    "YouTube - 720p", "YouTube - 1080p", "YouTube - 1440p", "YouTube - 2160p",
    "Vimeo - 720p", "Vimeo - 1080p", "Vimeo - 2160p",
    "TikTok - 720p", "TikTok - 1080p",
    "Presentations",
    "Dropbox - 720p", "Dropbox - 1080p", "Dropbox - 2160p",
    "Replay - 720p", "Replay - 1080p", "Replay - 2160p",
    "IMF - Generic", "IMF - 20th Century Fox", "IMF - Netflix",
    "IMF - Sony Pictures", "IMF - iQIYI", "IMF - HDR Vivid",
    "FCP - Final Cut Pro 7", "FCP - Final Cut Pro X",
    "Premiere XML", "Audio Only", "AVID AAF", "Pro Tools",
    "Tencent - IMF", "Tencent - MOV", "Tencent - MP4",
    "VR 180/360 - Meta Quest VR", "VR 180/360 - YouTube VR",
])

def custom_render_presets(all_presets):
    """Strips DaVinci's own stock presets out of a GetRenderPresetList()
    result, leaving only ones a project/studio actually added — that's
    the only kind ever relevant to this app, and the full stock list is
    long enough to bury the 1-3 presets someone's actually choosing
    between."""
    return [p for p in all_presets if p not in DAVINCI_BUILTIN_RENDER_PRESETS]

def resolve_render_preset_name(project, target_name, log_callback=None):
    """Finds the actual DaVinci render preset name to load, tolerating
    minor naming drift (case, extra whitespace, or a differently
    numbered/prefixed copy of the same preset) instead of requiring an
    exact string match against RENDER_PRESET. LoadRenderPreset silently
    no-ops on a miss — no exception, and its bool return value wasn't
    even being checked at either call site — so a stale preset name
    previously meant either a wrong, unnoticed render or (at best) a log
    line explaining why the tech-info banner came back as "Unknown".

    Tries, in order: exact match, then case/whitespace-insensitive match,
    then a normalized match that strips any leading "NN_"-style numbering
    prefix so e.g. "02_COLORED VFX 4444 XQ" still matches a preset
    someone renamed to "1_Colored VFX 4444 XQ" or "COLORED VFX 4444 XQ",
    then substring containment either direction (a preset with extra
    text wrapped around the real name, e.g. "SHOW_COLORED VFX 4444
    XQ_v2"). Last resort: if exactly one preset's name contains "vfx" at
    all, use it — a real rename ("Colored VFX Export") won't match any
    of the stricter tiers above, but "vfx" is the one word specific
    enough to this preset's purpose that a single hit is still a safe
    bet. Only fires when there's exactly one such preset — with two or
    more, guessing which one is correct would be worse than just failing
    with a clear list of what's actually there. Falls back to
    target_name unchanged if nothing matches at all, so behavior for
    anyone whose preset name is untouched stays identical."""
    def _log(msg):
        if log_callback:
            log_callback(msg)
    try:
        available = custom_render_presets(project.GetRenderPresetList() or [])
    except Exception:
        return target_name
    ...
    def _norm(s):
        return re.sub(r"\s+", " ", re.sub(r"^\d+[_\-\s]*", "", s.strip())).lower()

    target_norm = _norm(target_name)
    for name in available:
        if _norm(name) == target_norm:
            _log(f"  ⚠ Render preset \"{target_name}\" not found exactly — "
                 f"using \"{name}\" instead (name matched loosely).")
            return name

    for name in available:
        n = _norm(name)
        if target_norm in n or n in target_norm:
            _log(f"  ⚠ Render preset \"{target_name}\" not found — using closest "
                 f"match \"{name}\" instead.")
            return name

    vfx_candidates = [name for name in available if "vfx" in name.lower()]
    if len(vfx_candidates) == 1:
        _log(f"  ⚠ Render preset \"{target_name}\" not found — \"{vfx_candidates[0]}\" "
             f"is the only preset with \"VFX\" in its name, using that.")
        return vfx_candidates[0]

    _log(f"  ✗ No render preset matching \"{target_name}\" found in DaVinci Resolve. "
...
def get_render_preset_tech_info(project, render_preset, render_info=None,
    ...
    if project and render_preset:
        try:
            resolved_preset = resolve_render_preset_name(project, render_preset, log_callback)
            if not project.LoadRenderPreset(resolved_preset):
                _log(f"  ✗ DaVinci Resolve rejected render preset \"{resolved_preset}\".")
...
def render_single_clip(project, timeline, render_preset,
    ...
    """
    # render_preset is expected to already be resolved (see
    # resolve_render_preset_name, called once by the caller rather than
    # per-clip here). LoadRenderPreset silently no-ops rather than
    # raising on a bad name, so this was previously an unchecked call —
    # a wrong/missing preset meant every clip quietly rendered with
    # whatever settings DaVinci already had loaded, not a visible error.
    if log_callback and not project.LoadRenderPreset(render_preset):
        log_callback(f"    ✗ DaVinci Resolve rejected render preset \"{render_preset}\" — "
                     f"rendering with whatever preset was already active.")
...
class ExportEngine:
    ...
    def run_export(self, output_dir, stop_flag=None, start_index=0, screenshot_callback=None,
        ...
        # render_preset, if given, is whatever the user picked in the
        # Render Preset dropdown — an actual name pulled straight from
        # this project's GetRenderPresetList(), so it's already correct
        # and doesn't need resolving. Falls back to the old
        # exact/fuzzy-match-against-RENDER_PRESET behavior only if the
        # caller didn't pass one (e.g. any other code path that still
        # calls run_export directly). Resolved/used once, outside the
        # per-clip loop below — every clip uses the same preset, so
        # doing this per clip (a GetRenderPresetList() round trip to
        # DaVinci) on possibly hundreds of clips would be pure waste.
        resolved_render_preset = render_preset or resolve_render_preset_name(
            self.project, RENDER_PRESET, self.log)
        ...
        for track_index in sorted(clips_by_track.keys()):
            ...
            for item in track_clips:
                ...
                success = render_single_clip(
                    ...
                    render_preset=resolved_render_preset,
...
class VFXExporterApp(tk.Tk):
    def __init__(self):
        ...
        self.after(2000, self._schedule_periodic_update_check)
    ...
    def _center_window(self):
        ...
        if h < 100:
            h = 800
        # Reopen wherever the user last had it, if that position still
        # makes sense on this screen — a saved spot from a monitor
        # that's no longer connected (or absurd values from some other
        # corruption) falls back to the original centered/top-flush
        # default rather than launching partly or fully off-screen.
        settings = self._load_app_settings()
        saved_x, saved_y = settings.get("window_x"), settings.get("window_y")
        if (isinstance(saved_x, int) and isinstance(saved_y, int)
                and -w + 50 <= saved_x <= sw - 50 and 0 <= saved_y <= sh - 50):
            x, y = saved_x, saved_y
        else:
            x = (sw - w) // 2
            # Flush to the top of the screen (not vertically centered) —
            # the Episode Export tab grows much taller once chips are
            # populated, so starting as high as possible maximizes room
            # to grow before the window runs off the bottom of the screen.
            y = 20
    ...
    def _on_window_moved(self, event=None):
        """Persists the window's position (not size — height is always
        content-driven, see _center_window) so the next launch reopens
        wherever the user last had it. <Configure> fires on every size
        change too (banner show/hide, tab switches, RESET ALL's own
        resize dance), not just drags — debounced and only actually
        written when the position genuinely changed, so those don't
        spam settings.json."""
        if getattr(self, "_window_pos_save_pending", False):
            return
        self._window_pos_save_pending = True

        def _save():
            self._window_pos_save_pending = False
            x, y = self.winfo_x(), self.winfo_y()
            settings = self._load_app_settings()
            if settings.get("window_x") != x or settings.get("window_y") != y:
                self._save_app_setting("window_x", x)
                self._save_app_setting("window_y", y)
    ...
    def _canvas_dropdown(self, parent, variable, placeholder="Select...", command=None,
        ...
        def _redraw_label():
            ...
            if val:
                ...
                fill = TEXT_MUTED
            display_text = val if val else placeholder
            # Truncate with an ellipsis rather than letting long text (a
            # preset name, a track name, ...) run past its own box —
            # create_text has no built-in width/wrap-to-ellipsis option,
            # so this has to measure and cut manually. Reserves extra
            # room on the right for the arrow triangle below (drawn last,
            # so it'd otherwise sit on top of overflowing text rather
            # than the text stopping short of it).
            import tkinter.font as tkfont
            f = tkfont.Font(font=FONT_SMALL)
            max_text_w = w - 12 - (24 if not readonly else 8)
            if f.measure(display_text) > max_text_w:
                while display_text and f.measure(display_text + "…") > max_text_w:
                    display_text = display_text[:-1]
                display_text = (display_text + "…") if display_text else "…"
            c.create_text(12, h // 2, text=display_text,
        ...
        def _open_popup():
            ...
            max_visible_rows = 10
            popup_w = w
            content_h = row_h * len(rows_data)
            visible_h = min(content_h, row_h * max_visible_rows)
            ...
            container = tk.Frame(popup, bg=BORDER)
            ...
            pc = tk.Canvas(container, width=popup_w, height=visible_h, bg=BG_PANEL,
                            highlightthickness=0)
            pc.pack(side="left")

            import tkinter.font as tkfont
            row_font = tkfont.Font(font=FONT_SMALL)
            max_row_text_w = popup_w - 20
            for i, label in enumerate(rows_data):
                row_text = label
                if row_font.measure(row_text) > max_row_text_w:
                    while row_text and row_font.measure(row_text + "…") > max_row_text_w:
                        row_text = row_text[:-1]
                ...
                text_id = pc.create_text(10, y0 + row_h // 2, text=row_text, font=FONT_SMALL,
            ...
            total_w = popup_w
            if needs_scroll:
                # A list long enough to run off-screen used to just do
                # exactly that — the popup's own height was never capped,
                # so e.g. a project's full unfiltered preset list (30+
                # entries) could open a window taller than the display.
                pc.configure(scrollregion=(0, 0, popup_w, content_h))
                sb_w = 10
                sb = self._make_scrollbar(container, command=pc.yview, width=sb_w)
            ...
            popup.geometry(f"{total_w + 2}x{visible_h + 2}+{x}+{y}")
    ...
    def _build_vfx_tab(self, main):
        ...
        # Render Preset dropdown — populated from whatever's actually in
        # the connected DaVinci project (not hardcoded to RENDER_PRESET),
        # so a project with a renamed or differently-numbered copy of the
        # preset still lets the user pick the right one instead of just
        # hoping the fuzzy name-matching in resolve_render_preset_name
        # guessed correctly. Locked/placeholder until Connect to Resolve
        # populates it — see autofill() in _do_connect.
        tk.Label(btn_frame, text="Preset:", font=FONT_SMALL, bg=BG_DARK,
                 fg=TEXT_MUTED).pack(side="left", padx=(16, 6))
        self._render_preset_var = tk.StringVar(value="")
        self._render_preset_dropdown = self._canvas_dropdown(
            btn_frame, self._render_preset_var, placeholder="Render Preset",
            command=self._on_render_preset_selected, width=170)
        self._render_preset_dropdown.pack(side="left")
        self._render_preset_dropdown.set_locked(True)
        ...
        def task():
            try:
                ...
                success = self.engine.run_export(output,
                    ...
                    render_preset=self._render_preset_var.get() or None)
                ...
                if self._stop_export:
                    ...
                    if self._create_xlsx.get():
                        ...
                        _render_preset = self._render_preset_var.get() or RENDER_PRESET
                        _lf = _list_folder
                        _sm = dict(_shot_map)
    ...
    def _check_for_updates(self, manual=False):
        """Check GitHub for a newer version. Silent on the automatic path
        (startup + periodic background re-checks) — no update found or a
        flaky network blip shouldn't interrupt anyone. manual=True (the
        App menu's "Check for Updates...") always surfaces a result,
        since a click with no visible response just looks broken."""
        def task():
            try:
                ...
                if manual:
                    # Capture the message into a plain string before the
                    # lambda, not the exception object itself — Python
                    # auto-unbinds an `except ... as e` name the moment
                    # this block ends, so by the time self.after(0, ...)
                    # actually runs this lambda (later, on the Tk event
                    # loop), the closure's reference to `e` would already
                    # be gone, raising NameError instead of showing the
                    # error this dialog exists to report.
                    msg = str(e)
                    self.after(0, lambda: self._pp_alert_dialog(
                        "Couldn't Check for Updates", msg))
        threading.Thread(target=task, daemon=True).start()

    def _schedule_periodic_update_check(self):
        """Re-checks for updates every hour for as long as the app stays
        open, so a long-running session (or one left open overnight)
        picks up a new release without needing a restart — the original
        check only ever fired once, 2s after launch. Self-rescheduling
        rather than a fixed interval loop so it keeps going indefinitely.

        Respects Preferences > Automatically check for updates — reread
        fresh each firing (not cached) so toggling it in Preferences
        takes effect on the very next hourly tick without a restart.
        Manual checks (Finishing Tool > Check for Updates...) bypass this
        entirely and always run, since a deliberate click should never
        be a no-op."""
        if self._load_app_settings().get("auto_check_updates", True):
            self._check_for_updates()
        self.after(60 * 60 * 1000, self._schedule_periodic_update_check)

    def _show_update_banner(self, remote_version, notes, download_url):
        """Show a dismissable update banner below the title. Guarded
        against duplicates: with periodic re-checking, a second check
        firing while the same update is still unaddressed would otherwise
        stack a second banner on top of the first.

        Drawn as a rounded Canvas background (same technique as
        _rounded_btn — a rounded polygon on a canvas whose own bg matches
        the parent's, so the corners outside it blend away) with the
        actual row embedded via create_window and inset by PAD on every
        edge, rather than a plain tk.Frame flush against the window
        edges. The row's buttons are rounded too; Release Notes is drawn
        as an outline pill in the same green as the message text
        (BNR_FG) instead of a solid fill, so it reads as a secondary
        action next to Close/Update Now without introducing an
        unrelated color."""
        if getattr(self, "_update_banner", None) is not None and self._update_banner.winfo_exists():
            return
        BNR_BG      = "#1a3a1a"
        BNR_FG      = "#6fcf6f"
        BTN_BG      = "#2a6e2a"
        ...
        canvas = tk.Canvas(self._main_frame, bg=BG_OUTER, highlightthickness=0)
        canvas.pack(fill="x", pady=(0, 8))
        self._update_banner = canvas

        inner = tk.Frame(canvas, bg=BNR_BG)

        def _rounded_pts(x1, y1, x2, y2, r):
            return [x1+r,y1, x2-r,y1, x2,y1, x2,y1+r, x2,y2-r, x2,y2, x2-r,y2, x1+r,y2,
                    x1,y2, x1,y2-r, x1,y1+r, x1,y1]

        def _make_solid_btn(parent, text, cmd):
            # Smoothed polygon at r=8 — the exact same technique and
            # radius _rounded_btn uses for every other button in this
            # app, so Close/Update Now's curve matches app-wide instead
            # of the tighter arc-based curve Release Notes needs (a
            # smoothed r and an arc'd r of the same number don't look
            # equally round — see _rounded_stroke's comment).
            import tkinter.font as tkfont
            f = tkfont.Font(family="SF Pro Display", size=11)
            pad_x, pad_y = 10, 5
            w = f.measure(text) + pad_x * 2
            h = f.metrics("linespace") + pad_y * 2
            c = tk.Canvas(parent, width=w, height=h, bg=BNR_BG, highlightthickness=0)
            poly = c.create_polygon(_rounded_pts(0, 0, w, h, 8), fill=BTN_BG, outline="", smooth=True)
            c.create_text(w // 2, h // 2, text=text, font=f, fill="#ffffff")
            c.bind("<Enter>",           lambda e: c.itemconfig(poly, fill=BTN_HOV))
            c.bind("<Leave>",           lambda e: c.itemconfig(poly, fill=BTN_BG))
            c.bind("<ButtonPress-1>",   lambda e: c.itemconfig(poly, fill=BTN_PRS))
            c.bind("<ButtonRelease-1>", lambda e: (c.itemconfig(poly, fill=BTN_HOV), cmd()))
            return c

        def _rounded_fill(c, x1, y1, x2, y2, r, fill, tag):
            # Rounded-rect fill from 2 rectangles + 4 pie-slice corner
            # arcs — real circular corners, same primitives _redraw_tab_box
            # uses for the main tab panel's border/fill.
            ...
            c.create_arc(x1, y1, x1 + 2*r, y1 + 2*r, start=90, extent=90, fill=fill, outline="", tags=tag)
            c.create_arc(x2 - 2*r, y1, x2, y1 + 2*r, start=0, extent=90, fill=fill, outline="", tags=tag)
            c.create_arc(x2 - 2*r, y2 - 2*r, x2, y2, start=270, extent=90, fill=fill, outline="", tags=tag)
            c.create_arc(x1, y2 - 2*r, x1 + 2*r, y2, start=180, extent=90, fill=fill, outline="", tags=tag)

        def _rounded_stroke(c, x1, y1, x2, y2, r, outline, width, tag):
            # Rounded-rect OUTLINE from 4 straight lines + 4 true arcs —
            # the stroke-only equivalent of _rounded_fill. A smoothed
            # polygon outline (fill="", smooth=True) turned out to render
            # jagged/uneven at this thin a stroke width (confirmed
            # visually) since Tk's polygon smoothing is a spline
            # approximation, not a true circle — create_arc draws a real
            # circular arc instead, matching _redraw_tab_box's border.
            we, he = x2 - 1, y2 - 1
            lx, ty = x1 + 1, y1 + 1
            c.create_line(lx + r, ty, we - r, ty, fill=outline, width=width, tags=tag, capstyle="projecting")
            c.create_line(we, ty + r, we, he - r, fill=outline, width=width, tags=tag, capstyle="projecting")
            c.create_line(we - r, he, lx + r, he, fill=outline, width=width, tags=tag, capstyle="projecting")
            c.create_line(lx, he - r, lx, ty + r, fill=outline, width=width, tags=tag, capstyle="projecting")
            c.create_arc(we - 2*r, ty, we, ty + 2*r, start=0, extent=90, style="arc", outline=outline, width=width, tags=tag)
            c.create_arc(we - 2*r, he - 2*r, we, he, start=270, extent=90, style="arc", outline=outline, width=width, tags=tag)
            c.create_arc(lx, he - 2*r, lx + 2*r, he, start=180, extent=90, style="arc", outline=outline, width=width, tags=tag)
            c.create_arc(lx, ty, lx + 2*r, ty + 2*r, start=90, extent=90, style="arc", outline=outline, width=width, tags=tag)

        def _make_outline_btn(parent, text, cmd):
            import tkinter.font as tkfont
            f = tkfont.Font(family="SF Pro Display", size=11)
            pad_x, pad_y = 10, 5
            w = f.measure(text) + pad_x * 2
            h = f.metrics("linespace") + pad_y * 2
            # A true arc at a given r reads noticeably rounder than a
            # smoothed polygon at that same r (Tk's polygon smoothing is
            # a gentler spline approximation, not a real circle) — r=4
            # here is what visually matches the smoothed r=8 the solid
            # buttons use, not a literal r=8 arc.
            r = 4
            c = tk.Canvas(parent, width=w, height=h, bg=BNR_BG, highlightthickness=0)
            _rounded_fill(c, 0, 0, w, h, r, BNR_BG, "fill")
            _rounded_stroke(c, 0, 0, w, h, r, BNR_FG, 1.3, "stroke")
            c.create_text(w // 2, h // 2, text=text, font=f, fill=BNR_FG)
            c.bind("<Enter>",           lambda e: c.itemconfig("fill", fill=RN_HOV_FILL))
            c.bind("<Leave>",           lambda e: c.itemconfig("fill", fill=BNR_BG))
            c.bind("<ButtonPress-1>",   lambda e: c.itemconfig("fill", fill=BNR_BG))
            c.bind("<ButtonRelease-1>", lambda e: (c.itemconfig("fill", fill=RN_HOV_FILL), cmd()))
            return c
        ...
        _make_solid_btn(inner, "✕", _dismiss_banner).pack(side="left", padx=(0, 10))
        tk.Label(inner, text=f"✦ Version {remote_version} available",
                 font=("SF Pro Display", 11), bg=BNR_BG, fg=BNR_FG).pack(side="left", padx=(0, 8))
        _make_outline_btn(inner, "Release Notes",
                           lambda: self._show_release_notes(remote_version, notes)
                           ).pack(side="left")
        _make_solid_btn(inner, "UPDATE NOW", lambda: self._do_update(remote_version, download_url)
                         ).pack(side="right")

        # Measure the row once built, then size the canvas around it
        # (content height + PAD top/bottom) and embed the row via
        # create_window, inset by PAD on the left/right too — width is
        # re-applied on every <Configure> since the canvas itself
        # stretches to the window's width via pack(fill="x") but this
        # embedded window doesn't follow that automatically.
        inner.update_idletasks()
        canvas_h = inner.winfo_reqheight() + 2 * PAD
        canvas.configure(height=canvas_h)
        win_id = canvas.create_window(PAD, canvas_h // 2, window=inner, anchor="w",
                                       width=max(inner.winfo_reqwidth(), 10))

        def _redraw_banner_bg(event=None):
            w = canvas.winfo_width()
            if w < 10:
                return
            canvas.delete("bg")
            canvas.create_polygon(_rounded_pts(0, 0, w, canvas_h, R), fill=BNR_BG,
                                   outline="", smooth=True, tags="bg")
            canvas.tag_lower("bg")
            canvas.itemconfig(win_id, width=max(w - 2 * PAD, 10))
        canvas.bind("<Configure>", _redraw_banner_bg)
    ...
    def _update_render_presets(self, ctx, filenames=None, progress_cb=None):
        ...
        folder on launch. The caller (_do_update) surfaces that as a
        blocking alert rather than relying on this log line alone: this
        app restarts ITSELF a couple seconds after this returns, and the
        log pane doesn't survive that restart, so a log-only warning here
        was trivially easy to never see — which is exactly what "the
    ...
    def _prebuild_update_dialog(self):
        ...
        status_lbl.pack(anchor="w", pady=(0, 14), fill="x")
        status_font = tkfont.Font(font=FONT_SMALL)
        STATUS_MAX_W = 332

        def _set_status(text):
            t = text
            if status_font.measure(t) > STATUS_MAX_W:
                while t and status_font.measure(t + "…") > STATUS_MAX_W:
                    t = t[:-1]
                t = (t + "…") if t else "…"
            status_var.set(t)

        tk.Frame(body, bg=BORDER, height=1).pack(fill="x", pady=(0, 10))
        tk.Label(body, text="Don't quit Finishing Tool while this finishes.",
    ...
    def _do_update(self, remote_version, download_url):
        ...
        this used to run all its network/file I/O directly on the main
        thread, which is exactly why the app appeared to hang for a few
    ...
    def _setup_menu(self):
        """Build the full Mac menu bar — persists regardless of which
        window is focused. "apple", "window", and "help" are Tk's three
        specially-named menus; giving them those names is what makes
        macOS treat them natively (app menu position, auto-populated
        window list/Minimize/Zoom, system Help search) instead of
        rendering as plain top-level menus."""
        menubar = tk.Menu(self)

        # App menu — the Apple-required stub cascade IS this app's own
        # menu on macOS (Tk auto-titles it from the app name once named
        # "apple"); About/Preferences are wired in via Tk's Aqua Apple
        # Event bridge below (createcommand), not add_command, since
        # that's the only way to land them in this menu's native
        # position ahead of Services/Hide/Quit rather than as a plain
        # top-level menu of their own.
        apple_menu = tk.Menu(menubar, name="apple", tearoff=0)
        menubar.add_cascade(menu=apple_menu)
        self.createcommand("tkAboutDialog", self._show_about_dialog)
        self.createcommand("tk::mac::ShowPreferences", self._open_preferences_dialog)
        # Regular add_command items on the "apple"-named menu land in
        # this app's own menu alongside the About/Preferences hooks
        # above — Check for Updates lives here now instead of Help,
        # since it's about the app itself, not documentation/support.
        apple_menu.add_command(label="Check for Updates...", command=lambda: self._check_for_updates(manual=True))

        # Edit menu — missing until now. Cut/Copy/Paste just re-fire the
        # same <<Cut>>/<<Copy>>/<<Paste>> virtual events Tk's Entry/Text
        # widgets already answer to Cmd+X/C/V; Select All is hand-rolled
        # since Entry and Text don't both answer <<SelectAll>> the same
        # way out of the box. Without this menu present at all, macOS
        # can leave Cmd+C/V behaving inconsistently in text fields.
        edit_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Edit", menu=edit_menu)

        def _edit_virtual(seq):
            w = self.focus_get()
            if w is None:
                return
            try:
        ...
        edit_menu.add_command(label="Cut", accelerator="Cmd+X", command=lambda: _edit_virtual("<<Cut>>"))
        edit_menu.add_command(label="Copy", accelerator="Cmd+C", command=lambda: _edit_virtual("<<Copy>>"))
        edit_menu.add_command(label="Paste", accelerator="Cmd+V", command=lambda: _edit_virtual("<<Paste>>"))
        edit_menu.add_separator()
        edit_menu.add_command(label="Select All", accelerator="Cmd+A", command=_edit_select_all)

        # Window menu — standard macOS convention; Tk appends
        # Minimize/Zoom/the open-window list to a menu named "window"
        # automatically, below whatever's added here.
        window_menu = tk.Menu(menubar, name="window", tearoff=0)
        menubar.add_cascade(label="Window", menu=window_menu)
        window_menu.add_command(label="Show Log", command=self._show_log_for_active_tab)
        window_menu.add_command(label="Setup Guide", command=self._open_guide)
        ...
        help_menu.add_command(label="Release Notes", command=self._show_release_notes)
        help_menu.add_command(label="View Source on GitHub", command=self._open_github_source)
        help_menu.add_command(label="Reveal Support Folder in Finder", command=self._reveal_support_folder)

        self.config(menu=menubar)
        self.update_idletasks()

    def _show_log_for_active_tab(self):
        """Window > Show Log — routes to whichever tab's own floating
        log window is relevant, same toggle each tab's own SHOW LOG
        label already drives (open if closed, close if open)."""
        if self._active_tab == "vfx":
            self._toggle_log()
        else:
            self._pp_toggle_log()

    def _open_github_source(self):
        try:
            subprocess.Popen(["open", "https://github.com/esandijp-dotcom/finishing-tool"])
        except Exception as e:
            self._log(f"⚠ Could not open GitHub: {e}", "warn")

    def _reveal_support_folder(self):
        """Opens ~/Library/Application Support/Finishing Tool in Finder
        — where settings.json (render preset memory, update-check
        preference) actually lives, useful for a support conversation
        without walking someone through a hidden Library path by hand."""
        try:
            subprocess.Popen(["open", os.path.dirname(self._settings_path())])
        except Exception as e:
            self._log(f"⚠ Could not open folder: {e}", "warn")

    def _show_release_notes(self, version=None, notes=None):
        """Help > Release Notes shows the currently-installed version's
        notes (no args). The update banner reuses this same dialog for
        the AVAILABLE remote version's notes instead — passing them in
        rather than reading APP_RELEASE_NOTES, since that constant is
        this install's notes, not the update's.

        Own Toplevel rather than _pp_alert_dialog's generic wrapped
        Label — that read as one dense, jumbled paragraph once the notes
        grew into a real bullet list (literal "- " markers, no visual
        hierarchy). Parses "- " lines into an actual indented bullet
        list in a Text widget instead, blank-line-separated blocks as
        paragraphs, with the app's own scrollbar for longer histories."""
        v = version or APP_VERSION
        n = notes if notes is not None else APP_RELEASE_NOTES

        dlg = tk.Toplevel(self)
        dlg.withdraw()
        dlg.title("Release Notes")
        dlg.configure(bg=BG_DARK)
        dlg.resizable(False, False)
        dlg.transient(self)
        ...
        tk.Label(body, text="Release Notes", font=("SF Pro Display", 16, "bold"),
                 bg=BG_DARK, fg=ACCENT, anchor="w").pack(anchor="w")
        tk.Label(body, text=f"Version {v}", font=FONT_SMALL, bg=BG_DARK, fg=TEXT_MUTED,
                 anchor="w").pack(anchor="w", pady=(0, 12))
        tk.Frame(body, bg=BORDER, height=1).pack(fill="x", pady=(0, 14))

        text_wrap = tk.Frame(body, bg=BG_DARK)
        text_wrap.pack(fill="both", expand=True)
        text_box = tk.Text(text_wrap, width=54, height=16, bg=BG_DARK, fg=TEXT_PRIMARY,
                            font=FONT_LABEL, wrap="word", relief="flat", bd=0,
                            highlightthickness=0, padx=0, pady=0, cursor="arrow",
                            takefocus=0)
        text_box.pack(side="left", fill="both", expand=True)
        scroll = self._make_scrollbar(text_wrap, command=text_box.yview)
        scroll.pack(side="left", fill="y", padx=(8, 0))
        text_box.configure(yscrollcommand=scroll.set)

        # "## " lines are category headers (UI, Functionality, etc.) —
        # bold/accent, with breathing room above so each section reads
        # as its own group instead of one undifferentiated bullet dump.
        text_box.tag_configure("header", font=("SF Pro Display", 12, "bold"),
                                foreground=ACCENT, spacing1=10, spacing3=4)
        text_box.tag_configure("bullet", lmargin1=16, lmargin2=32, spacing1=1, spacing3=4)
        text_box.tag_configure("para", spacing1=2, spacing3=10)

        text_box.config(state="normal")
        body_text = (n or "").strip() or "No release notes available for this version."
        for block in body_text.split("\n\n"):
            for line in block.split("\n"):
                ...
                if line.startswith("## "):
                    ...
                    text_box.insert("end", "•  " + line[2:] + "\n", "bullet")
                else:
                    text_box.insert("end", line + "\n", "para")
        # Trim the trailing newline the loop above always leaves.
        text_box.delete("end-2c", "end-1c")
        text_box.config(state="disabled")

        btn_row = tk.Frame(body, bg=BG_DARK)
        btn_row.pack(anchor="e", fill="x", pady=(14, 0))
        close_btn = self._rounded_btn(btn_row, "Close", lambda: dlg.destroy(), accent=True)
    ...
    def _open_preferences_dialog(self):
        """App menu > Preferences (Cmd+,), hooked in via
        tk::mac::ShowPreferences in _setup_menu. Currently the one real
        global setting: whether the hourly background update check
        runs at all (manual Finishing Tool > Check for Updates always
        works regardless). Settings live in settings.json — see
        _settings_path — deliberately outside the folder the
        auto-updater overwrites, so this survives every update."""
        dlg = tk.Toplevel(self)
        dlg.withdraw()
        ...
        tk.Label(body, text="Preferences", font=("SF Pro Display", 16, "bold"),
                 bg=BG_DARK, fg=ACCENT, anchor="w").pack(anchor="w", pady=(0, 12))

        settings = self._load_app_settings()
        auto_check_var = tk.BooleanVar(value=settings.get("auto_check_updates", True))

        def _on_toggle(*_a):
            self._save_app_setting("auto_check_updates", auto_check_var.get())

        auto_check_var.trace_add("write", _on_toggle)
        check = self._canvas_checkbox(body, auto_check_var, "Automatically check for updates")
        check.pack(anchor="w")
        tk.Label(body, text="Checks once an hour in the background. You can always check "
                             "manually from Finishing Tool → Check for Updates…",
                 font=FONT_SMALL, bg=BG_DARK, fg=TEXT_MUTED, wraplength=360,
                 justify="left", anchor="w").pack(anchor="w", pady=(4, 16))
        ...
        def _do_reset_prefs():
            try:
                ...
                self._log(f"⚠ Could not reset preferences: {e}", "warn")
                return
            auto_check_var.set(True)

        tk.Label(body, text="Also resets the remembered render preset — the picker on the "
                             "VFX tab will fall back to auto-matching again next connect.",
                 font=FONT_SMALL, bg=BG_DARK, fg=TEXT_MUTED, wraplength=360,
                 justify="left", anchor="w").pack(anchor="w", pady=(0, 8))
        reset_btn = self._rounded_btn(body, "Reset Saved Preferences", _do_reset_prefs)
        reset_btn.pack(anchor="w", pady=(0, 16))
    ...
    def _enable_reset_btn(self):
        ...
        self.btn_reset._text = "RESET ALL"
        self.btn_reset._bg = "#2a6e2a"
        self.btn_reset._draw("#2a6e2a", fg="#FFFFFF")
        self.btn_reset._action = self._do_reset
    ...
    def _disable_reset_btn(self):
        """Disable the RESET ALL button - greyed out. Text goes grey too
        (#555555), matching every other disabled button's look — this
        canvas draws its own text separately from _rounded_btn's shared
        _set_btn_state, which was the one place that convention wasn't
        applied, leaving the label white even while "disabled"."""
        if not hasattr(self, 'btn_reset'):
            return
        self._reset_armed[self._active_tab] = False
        self.btn_reset._draw("#2a2a2a", fg="#555555")
        self.btn_reset.unbind("<Enter>")
        self.btn_reset.unbind("<Leave>")
        self.btn_reset.unbind("<ButtonPress-1>")
        self.btn_reset.unbind("<ButtonRelease-1>")
    ...
    def _do_reset(self):
        ...
        if self._active_tab == "vfx":
            ...
            self._xlsx_check.set_enabled(False)
            self._render_preset_dropdown.set_options([])
            self._render_preset_var.set("")
            self._render_preset_dropdown.set_locked(True)
            self.engine = None
            self.episode_markers = []
            self.export_list = []
            ...
            self.btn_scan._text = "Scan Episodes"
        ...
        self.btn_reset._text = "RESET ALL"
        self.btn_reset._bg = "#2a6e2a"
        self.btn_reset._draw("#2a2a2a", fg="#555555")  # show greyed out
    ...
    def _settings_path(self):
        """A tiny local prefs file, separate from the app's own resource
        files (main.py, presets, etc. next to the script) since those all
        get silently overwritten by the auto-updater — a setting saved
        there would just vanish on the next update. Application Support
        is never touched by that."""
        d = os.path.expanduser("~/Library/Application Support/Finishing Tool")
        try:
    ...
    def _save_app_setting(self, key, value):
        try:
            ...
            self._log(f"⚠ Could not save setting: {e}", "warn")

    def _on_render_preset_selected(self, label):
        """User manually picked a render preset from the dropdown —
        remember it (see _settings_path) so a project that consistently
        uses a non-default preset doesn't need re-picking every session."""
        self._save_app_setting("render_preset", label)
        ...
        def task():
            try:
                ...
                def autofill():
                    ...
                    # RENDER PRESET dropdown — populated from whatever's
                    # actually in THIS DaVinci project, not assumed.
                    # Default selection, in order: the user's own
                    # remembered pick (if it's still present here), else
                    # the closest match to RENDER_PRESET via the same
                    # exact/fuzzy logic used at render time, else just the
                    # first preset in the list so the dropdown is never
                    # left blank when there's at least one option.
                    try:
                        presets = custom_render_presets(self.engine.project.GetRenderPresetList() or [])
                    except Exception:
                        presets = []
                    self._render_preset_dropdown.set_options(presets)
                    if not presets:
                        self._log("⚠ No custom render presets found in this DaVinci project "
                                  "(only stock ones, which are hidden from this list).", "warn")
                    else:
                        saved = self._load_app_settings().get("render_preset", "")
                        if saved in presets:
                            default_preset = saved
                        else:
                            default_preset = resolve_render_preset_name(
                                self.engine.project, RENDER_PRESET,
                                lambda m: self._log(m, "muted"))
                            if default_preset not in presets:
                                default_preset = presets[0]
                        self._render_preset_var.set(default_preset)
                        self._render_preset_dropdown.set_locked(False)
    ...
    def _do_scan(self):
        ...
        def task():
            try:
                ...
                self.after(0, self._restore_reset_btn)
                # A successful scan relabels the button in place to "Rescan
                # Episodes" and leaves it enabled (rather than disabling it,
                # as before) — lets a re-click pick up markers added to the
                # timeline after the first scan, same convention as
                # "Connect to AME" -> "Rescan Episodes" on the Episode
                # Export tab (see _pp_connect_ame).
                def _enable_rescan():
                    self.btn_scan._text = "Rescan Episodes"
                    self._set_btn_state(self.btn_scan, True)
                self.after(0, _enable_rescan)
